...
function preload() {
  img = loadImage('../img/wave.jpg');
  imgForest = loadImage('../img/forest.jpeg');
  backgroundImg = img;
}
...
function setup() {
  ...
  for (let i = 0; i < 91; i++) {
    let b = new Boid(width / 2,height / 2,random(wordsArray));
    flock.addBoid(b);
  }
}
...
function draw() {
  if (frameCount % 600 == 0) {
    if (countImg == 1) {
      backgroundImg = imgForest;
    } else {
      backgroundImg = img;
    }
    countImg = (countImg + 1) % 2;
  }
...
Boid.prototype.render = function() {
  let c = map(this.position.y, 0, width, 0, 255);
  fill(255-c, 0, c*2);
  stroke(255-c, 0, c*2);
  push();
